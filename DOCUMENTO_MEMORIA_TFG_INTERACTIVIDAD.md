...
- **Interacción y función**: Permite al usuario activar o silenciar con un solo toque la totalidad de los efectos de sonido foley y ambientes atmosféricos diegéticos, otorgándole el control absoluto de su entorno auditivo.
- **Implementación técnica**: Dado que los navegadores modernos restringen severamente la reproducción automática de sonido (*autoplay policy*), se diseñó un motor de inicialización perezosa (`lazy audio unlock`). Durante el primer evento de interacción del usuario con la página (`pointerdown`, `click` o `touchstart`), el sistema despierta y desbloquea el contexto de audio. Al conmutar el botón de sonido, una variable de control booleano (`isMuted`) intercepta todas las llamadas del motor de sonido (`SFXEngine`). Si el usuario silencia el cuento, los audios en reproducción experimentan un desvanecimiento inmediato mediante funciones de *fade-out* para evitar chasquidos acústicos desagradables (*pop clicks*).
...
### 2.4. Sistema Diegético de Diálogos, Bocadillos de Cómic y Avatares Expresivos
- **Lo que se ve en pantalla**: En cada viñeta, el diálogo de los personajes no se imprime como un bloque de texto plano, sino enmarcado en una pastilla de cómic con fondo blanco puro, sombra proyectada nítida y un borde redondeado de 4 píxeles cuyo color codifica la identidad del personaje que habla: naranja vibrante para Gato, rosa chicle para Crunchy, marrón cálido para Oti, carmesí para Riolita y grafito volcánico para Basalto. En el lateral izquierdo de la pastilla descansa un avatar circular con la cabeza del personaje, cuya ilustración cambia dinámicamente según el estado emocional de la frase (sonrisa, asombro, enfado o grito).
- **Interacción y función**: Cuando el usuario interactúa con la escena (al tocar, pulsar o avanzar), los diálogos se suceden secuencialmente réplica a réplica al ritmo que marca el propio lector. En momentos de fuerte carga dramática, gritos cómicos o discusiones acaloradas (como la abducción en el vórtice o la disputa por la roca), la pastilla ejecuta una sacudida visual animada (*shake / rage*), comunicando con fuerza el tono emocional y la energía del personaje sin necesidad de pistas de audio hablado.
- **Implementación técnica**: El sistema se gestiona mediante una arquitectura de secuencias en JavaScript (`sceneSteps`). Cada intervención se define en un objeto con el emisor (`speaker`), la clase de avatar y estilo emocional (`subSpeaker`), el texto del parlamento (`text`) y la acción animada vinculada (`action`). La función controladora `setSceneSubtitle` actualiza el DOM de forma síncrona, reemplazando el texto interior e inyectando las clases CSS del personaje y de animación de impacto (`shout-rage` o `destacat`). Para garantizar una lectura cómoda y evitar desincronizaciones si el usuario pulsa repetidamente con rapidez, se implementó un sistema de protección temporal (*anti-debounce*) y un gestor de secuencias activas mediante tokens (`cancelAllSequences`), asegurando que cada réplica concluya visualmente y mantenga el orden estricto de la conversación.
- **Justificación de la decisión**: Al prescindir de pistas de voz hablada, se preserva una experiencia de lectura fluida, universal e íntima, propia del álbum ilustrado y del cómic clásico. Permite que niños, padres o educadores lean los textos en voz alta a su propio ritmo sin interferencias sonoras, mientras que la codificación cromática, las variantes gestuales de los avatares y las vibraciones dinámicas de los bocadillos transmiten de forma visual inmediata la personalidad, el tono humorístico y la complicidad entre los protagonistas.
...
- **Diálogos Interactivos de la Escena (`script.js`)**:
...
- **Diálogos Interactivos en el Vórtice (`script.js`)**:
